...
if ( '' !== $thumb && 'on' === $show_thumbnail ) {
	...
	$output    .= '<div class="post-media" style="background-image: url(' . esc_url( $back_thumb ) . ')" data-title="' . esc_html( get_the_title( $post_id ) ) . '">';
	$output    .= '<a href="' . esc_url( get_the_permalink( $post_id ) ) . '" target="' . $target . '" class="abs-url" title="' . esc_html( get_the_title( $post_id ) ) . '">' . esc_html( get_the_title( $post_id ) ) . '</a>';
	if ( 'on' === $use_overlay ) {
